...
> **目标**：帮助读者了解 JiuwenSwarm 的记忆系统架构，掌握记忆写入、检索和配置方法。
>
> **英文版：** [Memory](../en/Memory.md)
...
```
{workspace_dir}/
├── USER.md                 # 用户档案（位于 workspace 根目录，非 memory/ 下）
└── memory/
    ├── MEMORY.md           # 长期记忆
    └── daily_memory/
...
| 用户个人信息 | USER.md（位于 workspace 根目录） | write / edit 工具 | "身处上海"、"名字叫小九" |
| 日常笔记、运行上下文 | memory/daily_memory/YYYY-MM-DD.md | write / edit 工具 | "今天在老正兴吃了松鼠桂鱼"、"查询了上海天气" |
| 用户说"记住这个" | memory/daily_memory/YYYY-MM-DD.md | write 工具 | "记住我喜欢吃辣" |
...
相关内容详见下文[「6. 进阶：Agent Swarm 团队记忆」](#6-进阶agent-swarm-团队记忆)章节。
...
**配置方式**：
通过环境变量配置 Embedding 服务，详见下文[「7.1 内置记忆配置」](#71-内置记忆配置)章节。

---

## 6. 进阶：Agent Swarm 团队记忆
...
│   └── bob_workspace -> ~/.jiuwenswarm/bob_workspace/   # 旧成员 symlink
└── team-workspace/                        # 团队共享文件区（含团队共享记忆 TEAM_MEMORY.md）
```

> **说明**：团队共享记忆 `TEAM_MEMORY.md` 的实际位置由 `config.yaml` 中 `modes.team.<team_name>.memory.team_memory_dir` 配置；默认随团队工作区创建在 `team-workspace/` 下，而非独立的 `team-memory/` 子目录。

### 6.3 团队记忆自动提取

持久团队的 Leader 在一个 round 结束后会自动从本 round 的任务记录和团队消息中提炼出值得长期保留的内容，更新到 `TEAM_MEMORY.md`。
