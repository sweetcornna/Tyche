...
自演进配置控制 JiuwenSwarm 的 Skill 经验审查、保存和自动 Skill 创建建议能力。

![开启本地技能自动演进与沉淀](../assets/images/skill演进_开关.png)
...
| 配置项 | 对应配置 | 默认值 | 作用 |
| --- | --- | --- | --- |
| **本地技能自动演进与沉淀** | `react.evolution.skill_evolution` | `false` | 统一控制自动 Skill 创建建议与 Skill 演进。关闭后相关 Rails、工具、提示词、watcher 和 `/evolve` 指令均不可用 |
| **Reviewer Feedback 最低置信度** | `react.evolution.review_feedback_min_confidence` | `0.7` | 仅处理达到该置信度的 Reviewer Feedback 归因结果 |

**开启后的实际效果**：

- Single Agent 当前默认每 5 个符合条件的非 follow-up 任务轮次自检一次；Team Leader 在每次团队任务确认完成后自检当次执行
- 错误或用户纠正只是审查证据，不会必然生成经验
- Single Agent 和 Team Leader 经过审查的改进提案按 `auto_save` 配置进入用户审批或自动保存
- 在技能总谱页可查看演进历史和变更详情
- 通过 `/evolve <skill_name> [user_intent]` 可立即发起指定 Skill 的演进审查
...
> ⚠️ **升级提醒**：升级会按新模板同步配置结构，但不会将旧版 `enabled`、`auto_scan`、`skill_create` 或相关环境变量的值转换为 `skill_evolution`。升级前已启用相关能力时，请在升级后重新确认该开关。

> 📖 **详情**：关于自演进机制的详细说明，请参考 [Skill 自演进](Skill自演进.md) 文档。

### TTSE（FACT/TIP 双轨）

TTSE 与 Skill 正文演进相互独立，由 `react.ttse` 控制，仅 agent 模式挂载。详情见 [TTSE](TTSE.md)。

| 配置项 | 路径 | 默认值 | 说明 |
|--------|------|--------|------|
| **启用 TTSE** | `react.ttse.enabled` | `false` | 是否挂载 `TTSERail`（agent-core 缺失时 Host 降级跳过）；开启时自动拉起 `agent_observability` 采集轨迹 |
| **轨迹归纳** | `react.ttse.evolve_enabled` | `true` | 是否从轨迹归纳 FACT/TIP |
| **注入指引** | `react.ttse.inject_enabled` | `true` | 是否注入系统 prompt 指引 |
| **Auto-dream** | `react.ttse.dream_enabled` | `true` | 是否静默整理经验库 |
| **Embedding** | `react.ttse.embedding.*` | `${EMBED_*}` | 可选；三段齐全时混合召回，否则 BM25 |

`store_path`、`inject_mode`、以及 dream 的 interval / min_hours / ttl 由实现固定，不对用户开放。
