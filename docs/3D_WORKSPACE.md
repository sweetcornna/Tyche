...
持仓视图的圆环（BTC）与棱锥（ETH）可以直接操作。反馈只作用于显示，不读取也不改变任何数量。

- 悬停：形体抬起、放大并朝指针倾斜；圆环同时转向观察者，棱锥加速自转；指针显示为可抓取。
- 按住与拖动：按下时轻微压缩，拖动时随指针方向旋转；快速甩出后先惯性转动，减速后沿最短路径弹回原姿态。点击仍打开详情。
- 联动：悬停或聚焦持仓卡片会点亮对应形体，悬停形体也会高亮对应卡片；选中资产时底部环出现一次涟漪，已选资产保留一圈淡轨道。
- 数据：新的模拟成交、撤单等事件触发一次涟漪，警示类事件用警示色；持仓环长度随数据平滑过渡；无持仓的形体转速更慢。

命中区域是不可见球体，圆环中间的空洞同样可点。触屏上横向拖动旋转，纵向滑动仍滚动页面。弹簧与惯性逻辑在
`apps/web/src/scene-motion.js`，由 `apps/web/test/scene-motion.test.mjs` 覆盖。
