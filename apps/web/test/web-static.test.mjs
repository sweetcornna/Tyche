...
test('localized shell exposes workflow structure without changing safety controls', () => {
  ...
  for (const text of ['事件日志', '运行 workflow', '模拟设置', '独立测试网', '固定工作流']) assert.match(app, new RegExp(text))
...
test('neutral conversation shell and connected workflow remain accessible and responsive', () => {
  const app = fs.readFileSync(path.join(SOURCE, 'App.jsx'), 'utf8')
  const css = fs.readFileSync(path.join(SOURCE, 'styles.css'), 'utf8')
  for (const color of ['#ffffff', '#f6f6f6', '#202020', '#e5e5e5']) assert.match(css, new RegExp(color, 'i'))
  assert.match(css, /\.app-shell\s*\{[^}]*color:\s*var\(--text\)/s)
  assert.match(app, /useState\('light'\)/)
...
test('polling never overwrites connection drafts and technical controls are collapsed', () => {
  ...
  assert.match(app, /className="context-panel"/)
  assert.match(app, /<details className="advanced"><summary>会话与测试网状态/)
  assert.match(app, /className="primary-action run-workflow" onClick=\{runCycle\}/)
