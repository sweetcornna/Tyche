...
function PositionCard({ symbol, position, selected, linked, onSelect, onHover }) {
  const pnl = position?.unrealized_pnl
  return <button type="button" className={`position-card ${selected ? 'is-selected' : ''} ${linked ? 'is-linked' : ''}`} onClick={onSelect} onPointerEnter={() => onHover(true)} onPointerLeave={() => onHover(false)} onFocus={() => onHover(true)} onBlur={() => onHover(false)} aria-pressed={selected}>
...
export function SceneDock({ compact, workflow, selected, onSelectAgent, paperView, initialMode = 'workflow', inlineDetails = false, motionEnabled = true }) {
  ...
  // Cards and 3D shapes share one hover; a selection carries its time so only a fresh one ripples.
  const [hoverAsset, setHoverAsset] = useState(null)
  const [selection, setSelection] = useState(null)
  const selectAsset = (symbol) => { setAsset(symbol); setSelection({ symbol, at: performance.now() }) }
  ...
  const content = <section data-view={mode} className={`scene-dock ${tight ? 'is-compact' : ''} ${expanded ? 'is-expanded' : ''} ${stale ? 'is-stale' : ''}`} aria-label="场景">
    ...
    <div className={`scene-surface mode-${mode}`}>
      <SceneVisual mode={mode} decorative={tight} paused={paused || !motionEnabled} workflow={workflow} selected={selected} onSelectAgent={(role) => { onSelectAgent(role); if (expanded) setExpanded(false) }} paper={paper} events={events} seenEvents={seenEvents.current} selectedAsset={expanded || inlineDetails ? asset : null} hoverAsset={hoverAsset} onHoverAsset={setHoverAsset} selection={selection} onSelectAsset={(symbol) => { selectAsset(symbol); setExpanded(true) }} />
    </div>
    </div>
