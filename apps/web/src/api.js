...
export const controlApi = Object.freeze({
  ...
  paperScene: (csrf, signal) => request('/api/paper/scene', { signal, cache: 'no-store', headers: csrf ? { 'X-CSRF-Token': csrf } : {} }),
  paperSetup: (csrf) => get('/api/paper/setup', csrf),
  setupPaper: (input, csrf) => post('/api/paper/setup', input, csrf),
  conversations: (csrf) => get('/api/conversations', csrf),
  changeConversation: (input, csrf) => post('/api/conversations', input, csrf),
  selectChatModel: (model, effort, csrf) => post('/api/chat/model', { model, effort }, csrf),
  changeTheme: (theme, csrf) => post('/api/ui', { theme }, csrf),
  cancelDiscussion: (requestId, csrf) => post('/api/strategy/cancel', { request_id: requestId }, csrf),
  ...
  discussStrategy: (message, csrf, allocate = false, requestId) => post('/api/strategy/discuss', { message, ...(requestId ? { request_id: requestId } : {}), ...(allocate ? { allocate: true } : {}) }, csrf),
