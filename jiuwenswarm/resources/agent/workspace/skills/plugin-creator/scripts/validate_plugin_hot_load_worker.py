#!/usr/bin/env python3
"""L2 hot-load worker for plugin packages — runs in a dedicated subprocess.
...
async def _hot_load(pkg: Path) -> dict[str, Any]:
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="validate_plugin_"))
    agent = None
    ...
    try:
        ...
        try:
            sys_op_id = f"validate_plg_{uuid.uuid4().hex[:8]}"
            ...
            agent = DeepAgent(
                AgentCard(
                    ...
                    description="plugin package dry-run validator",
                )
            )
            agent.configure(
                DeepAgentConfig(
            ...
            if not hasattr(agent, "load_plugin"):
                raise RuntimeError(
                    "当前 openjiuwen 版本的 DeepAgent 没有 load_plugin 方法"
                )
        except Exception as exc:
            return _result(
                "skip",
        ...
        try:
            record = await agent.load_plugin(str(pkg))
        except Exception as exc:
            errors.append([f"load_plugin 失败: {exc}", ""])
        ...
        test_session = create_agent_session(
            session_id=f"validate_plugin_{uuid.uuid4().hex[:8]}",
            card=agent.card,
            close_stream_on_post_run=False,
        )
        await test_session.pre_run(inputs={"query": "plugin validation"})
        try:
            for ref in record.refs:
    ...
    if len(sys.argv) != 2:
        write_stdout(
            json.dumps(
                _result(
                    "skip",
                    skip_reason="Usage: validate_plugin_hot_load_worker.py <package-dir>",
                    skip_fix="由 validate_plugin.py 以子进程方式调用",
                ),
                ensure_ascii=False,
            )
            + "\n"
