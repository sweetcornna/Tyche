...
import type {
  ...
  TrajectoryRequest as TrajectoryRequestNumber,
  TrajectoryRequestConfig as AssistantRequestConfig,
  TrajectoryTurnModel,
  TrajectoryUsage,
...
export interface TrajectoryTableProps {
  /** Request numbers for the request groups visible in this loaded window. */
...
export type { TrajectoryRequest as TrajectoryRequestNumber, TrajectoryUsage } from '../trajectory/model.ts'
...
function recordRequestKey(record: TableRecord): string {
  return record.cell.requestRecordId ?? requestKey(record.turn, record.group)
}
...
function indexRequestNumbers(
  ...
  for (const request of sessionNumbers ?? []) {
    numbers.set(request.recordId ?? requestKey(request.turn, request.group), request.number)
  }
  let next = Math.max(0, ...numbers.values()) + 1
  const boundaryRecords = records
    .filter(record => boundaries.get(recordRequestKey(record)) === record.cell.index
...
function stateOf(record: TableRecord): RecordState {
  if (record.cell.status !== undefined) return record.cell.status
...
function UsageRows({ usage }: { usage: TrajectoryUsage | undefined }) {
  ...
  const uncachedInput = usage.input === undefined
    ? undefined
    : Math.max(0, usage.input - (usage.cacheRead ?? 0) - (usage.cacheWrite ?? 0))
...
function RecordedFacts({
  ...
  facts: TrajectoryRecordedFacts | undefined
  preview?: boolean
}) {
  if (facts === undefined) {
    return <p className={css.noPayload}>Recorded facts not available</p>
  }
  return (
    <JsonTree
      data={facts}
      label="Recorded trajectory facts JSON"
      className={preview ? css.jsonPreview : css.jsonPayload}
    />
  )
}
...
function detailTabs(record: TableRecord): readonly DetailTabItem[] {
  const otel = record.cell.traceDetail === undefined
    ? []
...
function ToolCatalog({ tools }: { tools: TrajectoryPromptSnapshot['tools'] }) {
  ...
  return record.cell.kind === 'message' && record.cell.assistantMetrics !== undefined
    ...
    : (
      <dl className={css.overview}>
        ...
        <div><dt>Timing source</dt><dd>{record.cell.timeSeconds === null ? 'Not available' : 'Recorded timestamps'}</dd></div>
      </dl>
    )
}
  ...
  if (request?.startedAt !== undefined) {
    ...
    return (
      <dl className={css.overview}>
        ...
        <div>
          <dt>Timing source</dt>
          <dd>{duration === null ? 'Recorded timestamps (running)' : 'Recorded timestamps'}</dd>
        </div>
      </dl>
    )
  }
  ...
  const selectedRequestUsage = selectedRequestInfo?.usage ?? (
    selectedRequestAssistant === undefined
      ...
      : {
        ...
        ...(selectedRequestAssistant.cell.total === undefined
          ? {}
              ...
              >
                ...
                && (record.turn === null || !collapsedTurns.has(record.turn))
                  ...
                  const requestInfo = request === undefined
                    ? undefined
                    : sessionRequestNumbers?.find(candidate => (
                      candidate.recordId ?? requestKey(candidate.turn, candidate.group)
                    ) === key)
                          ...
                          : record.collapsedSummary !== undefined
                            ...
                            : (
                              ...
                              >
                                ...
                                {stateOf(record) === 'running' && (
                                  <span className={css.runningElapsed} aria-label="Running">
                                    <span className={css.runningDot} aria-hidden="true" />
                                    {formatElapsedSeconds(liveElapsedSeconds(
                                      record.cell,
                                      nowMilliseconds,
                                    ))}
                                  </span>
      ...
      {(selectedRequest !== null
        ...
        >
          ...
          >
            {selectedRequest !== null
              ...
              <>
                ...
                <div className={css.overviewSections}>
                  ...
                  {selectedRequestFacts !== undefined && (
                    <OverviewSection label="Facts" onOpen={() => { activateTab('facts') }}>
                      <RecordedFacts facts={selectedRequestFacts} preview />
                    </OverviewSection>
                  )}
            ...
            {selectedRequest !== null && activeTab === 'facts' && (
              <RecordedFacts facts={selectedRequestFacts} />
            )}
            ...
            {typeof selected?.cell.traceDetail === 'object'
              && selected.cell.traceDetail !== null
              && activeTab === 'otel' && (
              <JsonTree data={selected.cell.traceDetail} label="OTLP export request" />
            )}
          </div>
        </aside>
      )}
