...
export function Menu({ open, anchor, items, selectedId, selectedIds, onSelect, onClose, align = 'start', side = 'bottom', portal = false, closeOnPointerLeave = false, dense = false, compact = false, getAnchorRect, footer, className }: {
  ...
  const list = open && (
    <div
      ref={listRef}
      className={clsx(css.list, dense && css.denseList, compact && css.compactList, scrollable && css.scrollable, portal && css.portal, portal && 'jiuwenTrajectoryPortal', side === 'top' && !portal && css.sideTop, align === 'end' && !portal && css.alignEnd)}
      data-trajectory-theme={portal ? colorMode : undefined}
