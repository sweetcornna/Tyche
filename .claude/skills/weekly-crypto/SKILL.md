...
Use a credential-free planning process. Before launching `.claude/workflows/crypto-analysis.mjs`, top-level deterministic code must run the configuration check and create `data/crypto_market.json` for the requested real `date` (`YYYY-MM-DD`) and `isoWeek` (`YYYY-Www`). The workflow itself rejects either USDT-M testnet credential before launching an agent.
