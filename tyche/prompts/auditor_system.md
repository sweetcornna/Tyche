...
Inputs: the rendered paper in <paper_text> above, and below: its LaTeX sections in <paper_latex> (the \citep/\citet keys there are the keys of <cited_evidence>), the computed <results_brief>, <cited_evidence> (the verified abstract and evidence cards for each cited key), and, when present, <experiment_reflection> (the experiment agent's own reading of the runs).
