...
@pytest.mark.parametrize("mode", ["manual", "auto"])
def test_update_permission_rail_does_not_create_rail_for_cron_session(monkeypatch, mode):
    created = []
    ...
    adapter._update_permission_rail({"permissions": {"enabled": True, "mode": mode}, "models": {}})
